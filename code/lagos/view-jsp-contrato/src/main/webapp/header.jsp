        <link rel="stylesheet" type="text/css" href="css/header.css"/>
        <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@500&display=swap" rel="stylesheet">
        <script src="js/header.js"></script>
    </head>
<body>
<header>
    <nav>
        <ul>
            <li class="logo"><img src="images/lagos-logo-semfundo.png" alt="Logo da empresa"></li>
            <span class="menu">
                        <li class="contratante"><a
                                href="personarlizar-interface.jsp">Configurações de Interface da Loja</a></li>
                        <li class="contratante"><a
                                href="gerir-lojas/gerir-chefes/cadastrar-chefe.jsp">Cadastrar Chefe</a></li>
                        <li class="contratante"><a
                                href="gerir-lojas/gerir-chefes/listar-chefes.jsp">Listar Chefe</a></li>
                        <li class="contratante"><a href="gerir-lojas/cadastrar-loja.jsp">Cadastrar Loja</a></li>
                        <li class="contratante"><a href="gerir-lojas/editar-loja.jsp">Editar Loja</a></li>
                        <li class="contratante"><a href="gerir-lojas/listar-lojas.jsp">Listar Loja</a></li>
                        <li class="contratante"><a href="visualizar-contrato.jsp">Visualizar Contratos</a></li>
                        <li class="gerente"><a href="#">Editar Contratos</a></li>
                        <li class="gerente"><a href="#">Criar Contratos</a></li>
                        <li class="gerente"><a href="#">Remover Contrato</a></li>
                    </span>
        </ul>
    </nav>
</header>
